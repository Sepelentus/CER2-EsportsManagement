...
class _JugadoresPageState extends State<JugadoresPage> {
  ...
  Future<void> updateJugador(int id, String nombre, String juego, int edad, String caracteristicas, int equipo_id) async {
          ...
          itemBuilder: (context, index) {
            ...
            child: Container(
              ...
              child: Column(
                children: [
                  ListTile(
                    title: Text(' ${jugadores[index]['nombre']}', style: TextStyle(fontSize: 18, 
                          fontFamily: 'Outfit',
                          fontWeight: FontWeight.bold,
                          
                          color: Color.fromARGB(255,229,203,93))),
                    leading: Icon(Icons.person_2_outlined, size: 35, color: const Color.fromARGB(255,229,203,93),),
                    trailing: Row(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            children: <Widget>[
              TextFormField(
                initialValue: jugadores[index]['nombre'],
                onChanged: (value) {
                  jugadores[index]['nombre'] = value;
                },
              ),
              TextFormField(
                initialValue: jugadores[index]['caracteristicas'],
                onChanged: (value) {
                  jugadores[index]['caracteristicas'] = value;
                },
              ),
              TextFormField(
                initialValue: jugadores[index]['edad'].toString(),
                onChanged: (value) {
                  jugadores[index]['edad'] = int.parse(value);
                },
              ),
              TextFormField(
                initialValue: jugadores[index]['equipo_id'].toString(),
                onChanged: (value) {
                  jugadores[index]['equipo_id'] = int.parse(value);
                },
              ),
          ...
          actions: <Widget>[
            ...
            TextButton(
              child: Text('Guardar'),
              onPressed: () async {
                String nombre = jugadores[index]['nombre'] ?? '';
                String juego = jugadores[index]['juego'] ?? '';
                int edad = jugadores[index]['edad'] ?? 0;
                String caracteristicas = jugadores[index]['caracteristicas'] ?? '';
                int equipo_id = jugadores[index]['equipo_id'] ?? 0;
                await updateJugador(jugadores[index]['id'], nombre, juego, edad, caracteristicas, equipo_id);
                Navigator.of(context).pop();
              },
            ),
          ],
